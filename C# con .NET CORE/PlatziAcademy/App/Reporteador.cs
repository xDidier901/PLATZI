...
namespace CoreEscuela.App
{
    public class Reporteador
    {
        ...
        public Dictionary<string, IEnumerable<AlumnoPromedio>> GetTopAlumnosPorAsignatura(int cantidad = 3)
        {

            var listaPromedioAlumnosPorAsignatura = GetPromeAlumnPorAsignatura();
            var resultado = new Dictionary<string, IEnumerable<AlumnoPromedio>>();

            foreach (var promediosPorAsignatura in listaPromedioAlumnosPorAsignatura)
            {
                var topAlumnos = promediosPorAsignatura.Value.OrderByDescending(x => x.promedio).Take(cantidad);
                resultado.Add(promediosPorAsignatura.Key, topAlumnos);
            }
